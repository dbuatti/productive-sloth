...
import { AetherSinkTask } from "@/types/scheduler"; // Updated type
import { useSchedulerTasks } from '@/hooks/use-scheduler-tasks';
import { showSuccess, showError } from "@/utils/toast";
import { calculateEnergyCost } from '@/lib/scheduler-utils';
import DatePicker from "./DatePicker"; // Import DatePicker
...
const formSchema = z.object({
  ...
  original_scheduled_date: z.date({ required_error: "Original scheduled date is required." }), // Added original_scheduled_date
});

type AetherSinkTaskDetailFormValues = z.infer<typeof formSchema>; // Updated type

interface AetherSinkTaskDetailDialogProps { // Updated name
  task: AetherSinkTask | null; // Updated type
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

const AetherSinkTaskDetailDialog: React.FC<AetherSinkTaskDetailDialogProps> = ({ // Updated name
  task,
  open,
  onOpenChange,
}) => {
  const { updateAetherSinkTaskDetails, completeAetherSinkTask, updateAetherSinkTaskStatus } = useSchedulerTasks(''); // Updated mutations
  const [calculatedEnergyCost, setCalculatedEnergyCost] = useState(0);

  const form = useForm<AetherSinkTaskDetailFormValues>({ // Updated type
    ...
    defaultValues: {
      ...
      original_scheduled_date: new Date(), // Default value
    },
  });
  ...
  useEffect(() => {
    if (task) {
      form.reset({
        ...
        original_scheduled_date: task.original_scheduled_date ? parseISO(task.original_scheduled_date) : new Date(), // Set original_scheduled_date
      });
  ...
  const handleSubmit = async (values: AetherSinkTaskDetailFormValues) => { // Updated type
    if (!task) return;

    try {
      if (values.is_completed !== task.is_completed) {
        if (values.is_completed) {
          await completeAetherSinkTask(task, values.duration || 0); // Updated mutation, pass durationUsed
        } else {
          await updateAetherSinkTaskStatus({ taskId: task.id, isCompleted: false }); // Updated mutation
        }
      }

      await updateAetherSinkTaskDetails({ // Updated mutation
        id: task.id,
        name: values.name,
        ...
        original_scheduled_date: format(values.original_scheduled_date, 'yyyy-MM-dd'), // Update original_scheduled_date
      });
      showSuccess("AetherSink task updated successfully!");
      onOpenChange(false);
    } catch (error) {
      showError("Failed to save AetherSink task.");
      console.error("Failed to save AetherSink task:", error);
    }
  };
      ...
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto p-6">
        <DialogHeader className="border-b pb-4 mb-6">
          <div className="flex items-center justify-between">
            <DialogTitle className="text-2xl font-bold">AetherSink Task Details</DialogTitle> {/* Updated title */}
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
            <div className="space-y-6">
              ...
              <FormField
                control={form.control}
                name="original_scheduled_date" // Added field
                render={({ field }) => (
                  <FormItem className="flex flex-col">
                    ...
                    <FormDescription>
                      The date this task was originally intended for.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
...
export default AetherSinkTaskDetailDialog;