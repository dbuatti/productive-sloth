...
const TaskCreationSchema = z.object({
  ...
  task_environment: z.enum(['home', 'laptop', 'away', 'piano', 'laptop_piano']).default('laptop'), // NEW: Add task_environment
});
...
const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({ defaultPriority, defaultDueDate, onTaskCreated }) => {
  ...
  const form = useForm<TaskCreationFormValues>({
    ...
    defaultValues: {
      ...
      energy_cost: calculateEnergyCost(DEFAULT_TASK_DURATION_FOR_ENERGY_CALCULATION, false, false, 'laptop'), // NEW: Pass taskEnvironment
      is_custom_energy_cost: false,
      task_environment: 'laptop', // NEW: Default task_environment
    },
    mode: 'onChange',
  });

  // NEW: Effect to update calculated energy cost when isCritical, isBackburner, or task_environment changes
  useEffect(() => {
    const subscription = form.watch((value, { name }) => {
      if (!value.is_custom_energy_cost && (name === 'isCritical' || name === 'isBackburner' || name === 'task_environment')) {
        const newEnergyCost = calculateEnergyCost(
          DEFAULT_TASK_DURATION_FOR_ENERGY_CALCULATION, 
          value.isCritical ?? false,
          value.isBackburner ?? false,
          value.task_environment ?? 'laptop' // NEW: Pass task_environment
        );
        ...
        const taskEnvironment = form.getValues('task_environment'); // NEW: Get task_environment
        const newEnergyCost = calculateEnergyCost(
          DEFAULT_TASK_DURATION_FOR_ENERGY_CALCULATION, 
          isCritical ?? false,
          isBackburner ?? false,
          taskEnvironment ?? 'laptop' // NEW: Pass task_environment
        );
        setCalculatedEnergyCost(newEnergyCost);
        form.setValue('energy_cost', newEnergyCost, { shouldValidate: true });
      }
  ...
  useEffect(() => {
    ...
    const initialTaskEnvironment = form.getValues('task_environment'); // NEW: Get initial task_environment
    const initialEnergyCost = calculateEnergyCost(DEFAULT_TASK_DURATION_FOR_ENERGY_CALCULATION, initialIsCritical, initialIsBackburner, initialTaskEnvironment);
  ...
  const onSubmit = (values: TaskCreationFormValues) => {
    ...
    const newTask: NewTask = {
      ...
      task_environment: task_environment, // NEW: Pass task_environment
    };
    ...
    addTask(newTask, {
      onSuccess: () => {
        ...
        form.reset({
          ...
          energy_cost: calculateEnergyCost(DEFAULT_TASK_DURATION_FOR_ENERGY_CALCULATION, false, false, 'laptop'), // Reset to default calculated
          is_custom_energy_cost: false,
          task_environment: 'laptop', // NEW: Reset task_environment
        });
      }
    });
  };
...
<problem file="src/pages/SettingsPage.tsx" line="462" column="45" code="17002">Expected corresponding JSX closing tag for 'FormLabel'.</problem>
</dyad-problem-report><think>**Fixing JSX Tags**

I'm currently addressing a TypeScript compile-time error within `SettingsPage.tsx`. It seems there's a mismatch; I've identified that the `<Label>` element is being utilized when the intended element is `<FormLabel>`. It's likely an improper self-closing tag situation. I'm focusing on rectifying this discrepancy to ensure the JSX structure is valid.


**Updating Form Labels**

I've resolved the TypeScript error in `SettingsPage.tsx`. The culprit was a mismatched JSX tag. The code incorrectly used a `<Label>` component within a `<FormLabel>` element, leading to the compiler complaining about the missing closing tag. I've corrected this to ensure the proper closing tag is present for the `<FormLabel>` element. It's now correctly formed as `<FormLabel>Last Name</FormLabel>`.


</think>You're right, that's a simple JSX syntax error! I've fixed the `FormLabel` tag in `src/pages/SettingsPage.tsx`.

<dyad-write path="src/pages/SettingsPage.tsx" description="Fixing JSX closing tag for FormLabel.">
