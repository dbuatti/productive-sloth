...
const ProgressOverviewCard: React.FC = () => {
  ...
  const { allTasks } = useTasks(); // allTasks now comes from AetherSink

  // The tasks_completed_today in profile is already updated by the completion mutations
  // So we don't need to re-filter allTasks here for completed tasks today.
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div className="border-t border-dashed border-border/50 sm:border-t-0 sm:border-l p-5 rounded-md bg-card border border-dashed border-border/50 flex flex-col justify-center items-center text-center">
            ...
            <p className="text-sm text-muted-foreground flex items-center gap-1">
              ...
              Earned <span className="font-bold text-foreground font-mono">+{profile.xp - (profile.xp - (profile.tasks_completed_today * 10))} XP</span> today! {/* This XP calculation is a placeholder, actual XP is tracked in profile */}
            </p>
          </div>
        </div>
      </CardContent>
