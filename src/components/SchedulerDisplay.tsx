            ...
            {finalDisplayItems.map((item) => {
              ...
              return (
                <div key={taskItem.id} className="relative group flex gap-2 mb-3">
                  ...
                  <div className={cn("flex-1 rounded-xl transition-all duration-300 relative overflow-hidden flex flex-col px-2 py-1", isActive ? "bg-primary/10" : "bg-card/40 hover:bg-primary/5", taskItem.isCompleted && "opacity-40 grayscale")} style={{ height: `${Math.max(duration * MINUTE_HEIGHT, MIN_TASK_HEIGHT_PX)}px`, borderLeft: `3px solid ${isActive ? 'hsl(var(--primary))' : `hsl(${hue} 70% 50%)`}` }} onClick={() => dbTask && (setSelectedTask(dbTask), setIsDialogOpen(true))}>
