...
const LOG_PREFIX = "[SCHEDULER_PAGE]";
...
const SchedulerPage: React.FC<{ view: 'schedule' | 'sink' | 'recap' }> = ({ view }) => {
  ...
  const handleRebalanceToday = useCallback(async () => {
    console.log(`${LOG_PREFIX} handleRebalanceToday called`);
  ...
  const handleReshuffleEverything = useCallback(async () => {
    console.log(`${LOG_PREFIX} handleReshuffleEverything called`);
    await handleAutoScheduleAndSort(sortBy, 'all-flexible', [], selectedDay);
  }, [handleAutoScheduleAndSort, selectedDay, sortBy]);

  const handleZoneFocus = useCallback(async () => {
    console.log(`${LOG_PREFIX} handleZoneFocus called with environments:`, selectedEnvironments);
  ...
  const handleDetailedInject = useCallback(() => {
    console.log(`${LOG_PREFIX} Opening CreateTaskDialog from SchedulerInput`);
  ...
  const handleFreeTimeClick = useCallback((startTime: Date, endTime: Date) => {
    console.log(`${LOG_PREFIX} Opening CreateTaskDialog from free time slot: ${format(startTime, 'HH:mm')} - ${format(endTime, 'HH:mm')}`);
    setCreateTaskDefaultValues({
      defaultPriority: 'MEDIUM',
      defaultDueDate: selectedDayAsDate,
