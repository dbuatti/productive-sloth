...
type WorkdaySettingsFormValues = z.infer<typeof workdaySettingsSchema>;

interface WorkdaySettingsDialogProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
}

const WorkdaySettingsDialog: React.FC<WorkdaySettingsDialogProps> = ({ isOpen, onOpenChange }) => {
  const { user, profile, isLoading: isSessionLoading, updateProfile } = useSession();

  const form = useForm<WorkdaySettingsFormValues>({
    resolver: zodResolver(workdaySettingsSchema),
    defaultValues: {
  ...
  const onSubmit = async (values: WorkdaySettingsFormValues) => {
    if (!user) {
      showError("You must be logged in to update workday settings.");
      return;
    }
    ...
    try {
      ...
      showSuccess("Workday settings updated successfully!");
      onOpenChange(false);
    } catch (error: any) {
      showError(`Failed to update workday settings: ${error.message}`);
      console.error("Workday settings update error:", error);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Workday Window Settings</DialogTitle>
          <DialogDescription>
            Define the default start and end times for your auto-scheduler.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
...
export default WorkdaySettingsDialog;