...
export interface RetiredTask {
  ...
  is_critical: boolean;
  created_at: string; // Changed from retired_at to created_at
}
...
export interface ScheduledBreakItem extends BaseScheduledItem {
  type: 'break';
  description?: string; // NEW: Added optional description for breaks
}
...
export type ScheduledItem = ScheduledTaskItem | ScheduledBreakItem | FreeSlotItem | ScheduledTimeOffItem;
...
export interface ScheduleSummary {
  ...
  // NEW: Properties for SchedulerDashboardPanel
  totalTasks: number;
  activeTime: {
    hours: number;
    minutes: number;
  };
  breakTime: number; // Total break duration in minutes
  sessionEnd: Date; // Represents the end of the last scheduled item or workday end
  extendsPastMidnight?: boolean; // NEW: Indicates if schedule extends past midnight
  midnightRolloverMessage?: string; // NEW: Message for midnight rollover
  criticalTasksRemaining: number; // NEW: Count of critical tasks remaining
}

export interface FormattedSchedule {
  items: ScheduledItem[];
  summary: ScheduleSummary; // Use the exported ScheduleSummary
  dbTasks: DBScheduledTask[]; // Added dbTasks back
}